...
for versuch in range (0,100) :
  ...
  for k,name in enumerate(P_names) :
    ...
    for organ_type in [3,4] :
      ...
      # move geometry so that all points are positive in the z direction
      geom[:,2] -= np.min(geom[:,2])
      #print(geom.shape)
      ...
      points = vtk.vtkPoints()
      points.SetNumberOfPoints(geom.shape[0])
      for i in range(geom.shape[0]) :
        points.SetPoint(i, geom[i, 0], geom[i, 1], geom[i, 2])
      ...
      # get bounds for the plant
      bounds = pd.GetBounds()

      #print(pd.GetPoints())
      writer = vtk.vtkXMLPolyDataWriter()
      writer.SetFileName("plants/"+name + "2_" + str(time) + "_" + str(organ_type)+"_"+str(versuch) + ".vtp")
